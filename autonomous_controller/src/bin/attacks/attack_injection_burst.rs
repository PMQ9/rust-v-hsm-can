...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!(
        "{}",
        "   ATTACK: Short Cycle Injection (Below Threshold)   "
            .red()
            .bold()
    );
    println!(
    ...
    println!(
        "{}",
        "⚠  WARNING: This is a security research tool".yellow()
    );
    ...
    println!(
        "{} Each cycle: {} malicious brake commands with INVALID MAC (sent at {}ms intervals)",
        "→".red(),
        FRAMES_PER_CYCLE,
        FRAME_DELAY_MS
    );
    println!(
        "{} Delay between cycles: {}ms (allows ~20 legitimate frames to reset counter)",
        "→".red(),
        CYCLE_DELAY_MS
    );
    println!(
        "{} Detection threshold: 3 consecutive MAC failures",
        "→".yellow()
    );
    println!(
        "{} Expected: Should NOT trigger error (stays below threshold)",
        "→".green()
    );
    println!();
    ...
    println!(
        "{} Sent {} total malicious brake commands across {} cycles",
        "→".red(),
        NUM_CYCLES * FRAMES_PER_CYCLE,
        NUM_CYCLES
    );
