...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!(
        "{}",
        "   Rear Right Wheel Speed Sensor      ".green().bold()
    );
    println!(
        "{}",
        "═══════════════════════════════════════".green().bold()
    ...
    let firmware_code = b"WHEEL_RR_FIRMWARE_v1.0.0";
    ...
    loop {
        ...
        let noise = (counter as f32 * 0.1 + 1.5).sin() * 0.5;
        let actual_speed = (speed + noise).max(0.0);

        let data = encoding::encode_wheel_speed(actual_speed);
        match hsm_client
            .create_secured_frame(can_ids::WHEEL_SPEED_RR, data.to_vec())
            .await
        {
            Ok(secured_frame) => {
