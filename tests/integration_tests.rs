use autonomous_vehicle_sim::{
    ...
    types::{CanFrame, CanId, EcuConfig, ArmVariant},
    ecu::Ecu,
};
...
async fn test_basic_can_communication() {
    ...
    // Send frame
    ecu.send_frame(test_frame.id, test_frame.data.clone()).await.unwrap();
...
async fn test_multiple_ecu_communication() {
    ...
    let frame1 = CanFrame::new(
        CanId::Standard(0x100),
        vec![0xFF],
        "ECU1".to_string(),
    );
    
    ecu1.send_frame(frame1.id, frame1.data.clone()).await.unwrap();
...
async fn test_bus_capacity() {
    ...
    let frame2 = CanFrame::new(
        CanId::Standard(0x124),
        vec![0x02],
        "TEST".to_string(),
    );
